...
mapCam2Vid = json.load(open('data/mapCam2Vid.json','r'))
...
def plotCamTrips(cname, ents, sessions, ptrips, tag=""):
    n1 = 0
    n2 = len(ents)
...
def plotlyPlot(x, primary, secondary=None, segLists=None, title='', show=True, mode='lines'):
    # x axis values
...
def plotCamTrips(gres, logName, outDir='./'):
    with open(os.path.join(outDir,'%s.html'%logName), 'w') as f:
        for res in gres:
            ...
            fig = plotlyPlot(x=ents[:,0], primary={'cam':ents[:,1]}, segLists=segLists, title=res['cam'])
...
def getCamMetrics(gres, logName, outDir=''):
    ...
    df.columns = ['name','avg.perOk','goodTrips(%)','goodTrips','#trips','camOveruse(%)','SD','cnts']
    ...
    for state in df.state.unique():
        ...
        lsumm  = ldf[ldf.columns[1:-2]].mean(axis=0)
        lsumm['count'] = len(ldf)
